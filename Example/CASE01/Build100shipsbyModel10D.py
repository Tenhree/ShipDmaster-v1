...
tainingmodel = torch.load("ShipinfoGAN10D.pth", map_location=device)
...
label_input = CommonMethod.sampleto_categorical(np.random.randint(0, opt.n_classes, ship_num), opt.n_classes).to(device)
code_input = 0.5 +  torch.rand(ship_num, opt.code_dim, device=device)* (0.9 - 0.5)
...
for i in range(ship_num):
    ...
    max_rete=CommonMethod.show_WL_curvaturerate(mypts,".//WLcurvaturerate10D//",i)
