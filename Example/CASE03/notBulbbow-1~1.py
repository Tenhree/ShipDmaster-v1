...
set_seed(42)
def truncated_normal(size, mean=0.0, std=1.0, trunc_min=-1, trunc_max=1, device='cpu', dtype=torch.float):
    x = torch.empty(size, device=device, dtype=dtype).normal_(mean, std)
    while True:
        mask = (x < trunc_min) | (x > trunc_max)
        if not mask.any():
            break
        x[mask] = torch.empty_like(x[mask]).normal_(mean, std)
    return x
...
tainingmodel = torch.load("../../ShipInfoGAN/TrainingMolde/ShipinfoGAN20D.pth", map_location=device)
ship_generator.load_state_dict(tainingmodel['generator'])
ship_generator.eval()

ship_num=6
z = truncated_normal(size=(ship_num, opt.latent_dim), mean=0.0, std=1.0, trunc_min=-1, trunc_max=1, device=device)
label_input = CommonMethod.sampleto_categorical(np.zeros(ship_num,dtype=int), opt.n_classes).to(device)
...
#build model
Geometry_path=".//Geometry20D_notBulbbow-1~1//"
if not os.path.exists(Geometry_path):
    os.mkdir(Geometry_path)
PC_path = ".//PC20D_notBulbbow-1~1//"
...
for i in range(ship_num):
    ...
    max_rete=CommonMethod.show_WL_curvaturerate(mypts,".//WLcurvaturerate20D_notBulbbow-1~1//",i)
    Max_curvaturerate.append(max_rete)
#Post-processing
DisplacementError = (np.ravel(Displacement) - np.ravel(input_displacement)) / np.ravel(input_displacement)
combined = zip(Displacement,input_displacement,DisplacementError, Max_curvaturerate)
# to file
result_path=".//result20D_notBulbbow-1~1//"
if not os.path.exists(result_path):
    os.mkdir(result_path)
with open(result_path+"output.csv", 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['Displacement','input_displacement','DisplacementError', 'Max_curvaturerate'])
