...
def truncated_normal_pos(size, mean=0.0, std=1.0, trunc_min=1.0, device='cpu', dtype=torch.float):
    # 初始化为正态分布
    x = torch.empty(size, device=device, dtype=dtype).normal_(mean, std)

    # 拒绝采样：只保留 >= trunc_min 的值
    while True:
        mask = x < trunc_min
        if not mask.any():
            break
...
ship_num=6
z = truncated_normal_pos(size=(ship_num, opt.latent_dim),mean=0.0, std=1.0, trunc_min=1.0, device=device, dtype=torch.float)
...
#build model
Geometry_path=".//Geometry_notBulbbow1~inf//"
if not os.path.exists(Geometry_path):
    os.mkdir(Geometry_path)
PC_path = ".//PC_notBulbbow1~inf///"
...
# to file
result_path=".//result_notBulbbow1~inf///"
