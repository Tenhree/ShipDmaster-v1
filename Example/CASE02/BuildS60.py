...
ship_generator.eval()
data = np.loadtxt("S60.csv", delimiter=',')
...
x = [3.17761627,  9.11132104,  9.60328951, -0.26309479,  4.22657942,  9.78819713,
  3.04465925, -9.97938678 , 2.62653162, -9.02498096,  2.97094201,  9.92790937,
  0.45339588 , 5.31061104 , 2.12140013, -1.63472217,  8.18056709,  2.85487396,
  1.95098717, -8.86562168 , 0.89879341] #ship 2
z = torch.tensor(x[0:20]).unsqueeze(0).float().to(device)
code_input = x[20] * torch.ones(1, opt.code_dim, device=device)
label_input2 = CommonMethod.sampleto_categorical(np.zeros(1, dtype=int), opt.n_classes).to(device)

ships = ship_generator(z, label_input2, code_input)
...
shipDis = myshipdecoder.Pix2hull(mypts, Lwl, Dd, Bd, 40, 40, 40 - wfb[0], Geometry_path, "S60fromShipInfoGAN.stl",
                                 PC_path, "S60fromShipInfoGAN.csv")
